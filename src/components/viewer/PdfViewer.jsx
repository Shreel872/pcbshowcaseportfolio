...
// ─────────────────────────────────────────────────────────────
// PDF viewer with explicit page navigation, zoom, fullscreen,
// and open-in-new-tab. Uses the browser's native PDF renderer
// inside an iframe so there's no extra JS bundle, but layers a
// custom toolbar on top so the experience is consistent across
// Chrome/Firefox/Edge regardless of how each one decides to
// render its built-in toolbar.
// ─────────────────────────────────────────────────────────────
...
export default function PdfViewer({
  ...
  // Reset to initial page if pdfPath changes
  useEffect(() => {
    setCurrentPage(page);
  }, [pdfPath, page]);
  ...
  // view=FitH gives a proper fit-to-width on first load
  ...
  return (
    <div
      ref={containerRef}
      className={`relative flex flex-col bg-gray-950 ${className}`}
    >
      {/* Toolbar */}
      <div className="flex items-center justify-between flex-wrap gap-2 px-3 py-2 border-b border-gray-800/70 bg-gray-900/80 backdrop-blur-sm shrink-0">
        ...
        {/* Centre: title */}
        <div className="hidden sm:block text-[11px] text-gray-500 font-medium truncate max-w-[40%]">
          {title}
        </div>

        {/* Right: zoom + external actions */}
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            onClick={resetZoom}
            className={`${btnGhost} font-mono w-12`}
            title="Reset zoom"
          >
            {zoom}%
          </button>
